...
Об’єктами дослідження (розробки) в кваліфікаційній роботі за освітньо-професійною програмою "Комп’ютерні технології та програмування в автоматизованих системах управління" рівня "магістр" можуть бути автоматизовані системи керування виробництвом, технологічним комплексом та технологічними процесами чи обладнанням, які розробляться в даній роботі. 

Предмет — конкретне технічне рішення, принцип побудови, алгоритм, архітектура, програмний модуль, засоби інтеграції, методика налаштування тощо. Тобто те, на що спрямована інженерна робота. Об'єкти дослідження можуть співпадати у роботах здобувачів, тоді як предмети залежать від спеціального завдання. 

## 5.2. Структура кваліфікаційної роботи
...
Приклад змісту можна знайти за наступним посиланням [Методичні вказівки до виконання курсової роботи MOM](https://github.com/pupenasan/MOMdisc/blob/main/%D0%BA%D1%83%D1%80%D1%81%D0%BE%D0%B2%D0%B8%D0%B9/method.md)
...
### Теми зі спеціальними завданнями, орієнтованими на виконання спеціалізованої функції в складі системи керування з використанням класичних або інтелектуальних систем керування 

Дана тема передбачає виконання завдання по створенню конкретної підсистеми керування в складі ІСКВ, а також реалізацією певної функції, яка визначається спеціальним завданням. Прикладами такої функції є (але не обмежено ними):

- просунутої аналітики
- автоматизованого керування на основі класичних алгоритмів:
  - адаптивного
  - лінійно-квадратичного керування
  - лінійно-квадратичного гаусівського керування
  - спостерігачів стану (в т.ч. фільтр Калмана)
  - робастного керування
  - MPC
  - методів нелінійної динаміки   
- автоматизованого керування на основі інтелектуальних алгоритмів:
  - нечіткого керування, 
  - нейромережевого керування
  - за прецедентами
  - RL
  - MPC
  - еволюційних методів
...
- інформаційну структуру системи керування  (обраної підсистеми)
- структуру та опис програмного забезпечення та його компонентів, яке використовується для побудови системи або підсистеми
- фрагменти програм та дисплейних кадрів для PLC, SCADA або IIoT, які реалізовують наведені алгоритми    

### Теми зі спеціальними завданнями, орієнтованими на використання робототехнічних комплексів.

Дана тема передбачає виконання завдання по створенню конкретної підсистеми керування в складі ІСКВ, а також реалізацією завдання з використанням промислових роботів.
...
- схему з’єднань та підключень проводок промислових мереж (за необхідності)
- принципові електричні схеми (за необхідності)
- інформаційну структуру системи керування (обраної підсистеми), зокрема опис інтерфейсу обміну з PLC або іншими підсистемами, наприклад обмін сигналами по мережі, отримання бітів синхронізації з системою безпеки, передача сигналу Robot_Ready, тощо).
- структуру та опис програмного забезпечення та його компонентів, яке використовується для побудови системи або підсистеми
...
Опис інтерфейсу обміну з PLC або іншими підсистемами

- Обмін сигналами по PROFINET.
- Отримання бітів синхронізації з системою безпеки.
- Передача сигналу Robot_Ready.

### Теми зі спеціальними завданнями, орієнтованими на використання цифрових двійників 

Дана тема передбачає виконання завдання по створенню конкретної підсистеми керування в складі ІСКВ, а також реалізацією певної функції з використанням цифрового двійника (**ЦД**).

У третьому розділі рекомендується навести: 

- опис та результати дослідження об'єкту, виокремлення задачі двійникування  
- схему автоматизації підсистеми керування (за необхідності)
- опис структури ЦД та її складових
- обґрунтування методів створення моделей для цифрового двійника
- реалізація моделі та перевірка моделі на адекватність
- алгоритми прийняття рішень на основі моделі ЦД

У четвертий розділ рекомендується включати 

- інформаційну структуру системи керування з інтегрованим ЦД
- структуру та опис програмного забезпечення та його компонентів, яке використовується для реалізації ЦД та інтеграції з іншими підсистемами
- фрагменти відеокадрів системи людино-машинного інтерфейсу з ЦД 

### Теми зі спеціальними завданнями, орієнтованими на побудову інтелектуальних систем

Дана тема передбачає виконання завдання по створенню інтелектуальної системи керування (ІСК). Детально концепція розроблення ІСК та виконання окремих розділів кваліфікаційної роботи описано в [6].

У третьому розділі рекомендується навести: 

- опис та результати дослідження об'єкту
- означення вимог до ІСК
...
- Розробка BDD технологічної, технічної та інформаційної складових системи
- інформаційну структуру системи керування 
- структуру та опис програмного забезпечення та його компонентів, 
- фрагменти відеокадрів системи людино-машинного інтерфейсу
